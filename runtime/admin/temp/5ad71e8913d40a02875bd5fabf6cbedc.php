<?php /*a:2:{s:62:"/www/wwwroot/2026/08/16/17_3/app/admin/view/member/detail.html";i:1787185769;s:55:"/www/wwwroot/2026/08/16/17_3/app/admin/view/layout.html";i:1787106485;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>会员详情 - 竞拍商城</title>
<style>
...
<div class="layout">
    ...
    <div class="main">
        <header class="topbar">
            <div class="breadcrumb">会员管理 / <a href="/admin1314/member/index">会员列表</a> / <b>会员详情</b></div>
...
<div class="card">
    ...
    <table class="detail-list">
        <tr><td>会员ID</td><td><?php echo htmlentities((string) $user['id']); ?></td></tr>
        <tr><td>手机号</td><td><?php echo htmlentities((string) $user['mobile']); ?></td></tr>
        <tr><td>昵称</td><td><?php echo !empty($user['nickname']) ? htmlentities((string) $user['nickname']) : '-'; ?></td></tr>
        <tr><td>邀请码</td><td><?php echo htmlentities((string) $user['invite_code']); ?></td></tr>
        <tr><td>会员类型</td>
            <td><?php if($user['is_virtual'] == 1): ?><span class="tag tag-purple">虚拟会员</span><?php else: ?><span class="tag tag-gray">普通会员</span><?php endif; if($user['is_seller'] == 1): ?><span class="tag tag-orange">卖家</span><?php endif; if($user['seller_check'] == 0): ?><span class="tag tag-orange">待审核</span>
                <?php elseif($user['seller_check'] == 1): ?><span class="tag tag-green">已审核</span>
                <?php else: ?><span class="tag tag-red">已拒绝</span><?php endif; ?>
            </td>
        </tr>
        <?php if(!empty($user['shop_name'])): ?>
        <tr><td>店铺名称</td><td><?php echo htmlentities((string) $user['shop_name']); ?></td></tr>
        <tr><td>企业名称</td><td><?php echo !empty($user['company_name']) ? htmlentities((string) $user['company_name']) : '-'; ?></td></tr>
        <tr><td>店铺介绍</td><td><?php echo !empty($user['seller_intro']) ? htmlentities((string) $user['seller_intro']) : '-'; ?></td></tr>
        <tr><td>消费保证金</td><td class="price">￥<?php echo htmlentities((string) $user['deposit']); ?></td></tr>
        <tr><td>店铺评分</td><td><?php echo htmlentities((string) $user['shop_score']); ?></td></tr>
        <tr><td>粉丝数量</td><td><?php echo htmlentities((string) $user['fans_count']); ?></td></tr>
        <tr><td>店铺资料</td>
            <td><button class="btn btn-sm btn-primary" onclick="openShopEdit()">编辑店铺资料</button></td>
        </tr>
        <tr><td>营业执照</td>
            <td>
                <?php if(!empty($user['lic_list'])): foreach($user['lic_list'] as $li): ?><img src="<?php echo htmlentities((string) $li); ?>" style="height:80px;border-radius:6px;cursor:pointer;margin-right:6px;" onclick="window.open('<?php echo htmlentities((string) $li); ?>')"><?php endforeach; else: ?>-
                <?php endif; ?>
            </td>
        </tr>
        <?php endif; ?>
        <tr><td>可用余额</td><td class="price">￥<?php echo htmlentities((string) $user['balance']); ?></td></tr>
        <tr><td>冻结余额</td><td>￥<?php echo htmlentities((string) $user['freeze_balance']); ?></td></tr>
        <tr><td>积分</td><td><?php echo htmlentities((string) $user['points']); ?></td></tr>
        <tr><td>注册时间</td><td><?php echo htmlentities((string) $user['reg_time_text']); ?></td></tr>
        <tr><td>最后登录</td><td><?php echo htmlentities((string) $user['last_login_text']); ?></td></tr>
        <tr><td>注册IP</td><td><?php echo !empty($user['reg_ip']) ? htmlentities((string) $user['reg_ip']) : '-'; ?></td></tr>
        <tr><td>会员状态</td><td><?php if($user['status'] == 1): ?><span class="tag tag-green">正常</span><?php else: ?><span class="tag tag-red">禁用</span><?php endif; ?></td></tr>
        <tr><td>统计</td><td>卖出 <?php echo htmlentities((string) $user['sell_count']); ?> 单 / 买入 <?php echo htmlentities((string) $user['buy_count']); ?> 单 / 出价 <?php echo htmlentities((string) $user['bid_count']); ?> 次</td></tr>
    </table>
</div>
...
<div class="card">
    ...
    <div class="table-wrap">
        <table class="table">
            ...
            <tbody>
            <?php if(empty($pay_accounts) || (($pay_accounts instanceof \think\Collection || $pay_accounts instanceof \think\Paginator ) && $pay_accounts->isEmpty())): ?>
            <tr><td colspan="6" class="empty">未绑定提现账户</td></tr>
            <?php else: if(is_array($pay_accounts) || $pay_accounts instanceof \think\Collection || $pay_accounts instanceof \think\Paginator): $i = 0; $__LIST__ = $pay_accounts;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$p): $mod = ($i % 2 );++$i;?>
            <tr>
                <td><span class="tag tag-blue"><?php echo htmlentities((string) $p['type_name']); ?></span></td>
                <td><?php echo !empty($p['real_name']) ? htmlentities((string) $p['real_name']) : '-'; ?></td>
                <td><?php echo htmlentities((string) $p['account']); ?></td>
                <td><?php echo !empty($p['bank_name']) ? htmlentities((string) $p['bank_name']) : '-'; ?></td>
                <td><?php if($p['qr_code']): ?><img src="<?php echo htmlentities((string) $p['qr_code']); ?>" style="height:36px;border-radius:4px;cursor:pointer;" onclick="window.open('<?php echo htmlentities((string) $p['qr_code']); ?>')" title="点击查看大图"><?php else: ?>-<?php endif; ?></td>
                <td><?php echo htmlentities((string) date('Y-m-d H:i:s',!is_numeric($p['create_time'])? strtotime($p['create_time']) : $p['create_time'])); ?></td>
            </tr>
            <?php endforeach; endif; else: echo "" ;endif; ?>
            <?php endif; ?>
            </tbody>
...
<div class="card">
    ...
    <div class="table-wrap">
        <table class="table">
            ...
            <tbody>
            <?php if(empty($logs) || (($logs instanceof \think\Collection || $logs instanceof \think\Paginator ) && $logs->isEmpty())): ?>
            <tr><td colspan="5" class="empty">暂无流水</td></tr>
            <?php else: if(is_array($logs) || $logs instanceof \think\Collection || $logs instanceof \think\Paginator): $i = 0; $__LIST__ = $logs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?>
            <tr>
                <td>
                    <?php switch($l['type']): case "recharge": ?><span class="tag tag-green">充值</span><?php break; case "deposit": ?><span class="tag tag-blue">保证金</span><?php break; case "buy": ?><span class="tag tag-orange">支付货款</span><?php break; case "income": ?><span class="tag tag-green">卖出收入</span><?php break; case "refund": ?><span class="tag tag-gray">退回</span><?php break; case "withdraw": ?><span class="tag tag-red">提现</span><?php break; default: ?><span class="tag tag-gray"><?php echo htmlentities((string) $l['type']); ?></span>
                    <?php endswitch; ?>
                </td>
                <td class="<?php if($l['amount'] >= 0): ?>price<?php endif; ?>"><?php echo $l['amount']>0 ? '+'  :  ''; ?><?php echo htmlentities((string) $l['amount']); ?></td>
                <td>￥<?php echo htmlentities((string) $l['balance']); ?></td>
                <td><?php echo htmlentities((string) $l['remark']); ?></td>
                <td><?php echo htmlentities((string) date('Y-m-d H:i:s',!is_numeric($l['create_time'])? strtotime($l['create_time']) : $l['create_time'])); ?></td>
            </tr>
            <?php endforeach; endif; else: echo "" ;endif; ?>
            <?php endif; ?>
            </tbody>
...
<div class="card">
    ...
    <div class="table-wrap">
        <table class="table">
            ...
            <tr>
                <td><?php echo htmlentities((string) $b['goods_title']); ?></td>
                <td class="price">￥<?php echo htmlentities((string) $b['price']); ?></td>
                <td><?php if($b['is_winner'] == 1): ?><span class="tag tag-green">得标</span><?php else: ?><span class="tag tag-gray">未得标</span><?php endif; ?></td>
...
<script>
var shopUserId = <?php echo htmlentities((string) $user['id']); ?>;
var shopData = {
    intro: <?php echo json_encode($user['seller_intro']); ?>,
    deposit: '<?php echo htmlentities((string) $user['deposit']); ?>',
    score: '<?php echo htmlentities((string) $user['shop_score']); ?>',
    fans: '<?php echo htmlentities((string) $user['fans_count']); ?>'
};

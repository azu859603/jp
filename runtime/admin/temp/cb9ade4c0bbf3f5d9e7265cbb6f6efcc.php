<?php /*a:2:{s:49:"D:\work\08\17_3\app\admin\view\balance\index.html";i:1787106133;s:42:"D:\work\08\17_3\app\admin\view\layout.html";i:1787106090;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>余额流水 - 竞拍商城</title>
<style>
...
<div class="layout">
    ...
    <div class="main">
        <header class="topbar">
            <div class="breadcrumb">财务管理 / <b>余额流水</b></div>
...
<div class="search-bar">
    <input type="text" class="form-control" id="keyword" placeholder="会员ID/手机号">
    <select class="form-control" id="type">
        <option value="">全部类型</option>
        <option value="recharge">充值</option>
        <option value="deposit">保证金</option>
        <option value="buy">支付货款</option>
        <option value="income">卖出收入</option>
...
<div class="card">
    <div class="table-wrap">
        <table class="table">
            <thead>
            <tr>
                <th>ID</th><th>会员</th><th>类型</th><th>变动金额</th><th>变动后余额</th><th>备注</th><th>时间</th>
            </tr>
            </thead>
...
<script>
var typeMap = {
    recharge: ['充值','tag-green'], deposit: ['保证金','tag-blue'], buy: ['支付货款','tag-orange'],
    income: ['卖出收入','tag-green'], refund: ['退回','tag-gray'], withdraw: ['提现','tag-red'], reward: ['奖励','tag-purple']
};

function loadList(page){
    fetch('/admin1314/balance/index?page=' + page + '&limit=15'
        + '&keyword=' + encodeURIComponent(document.getElementById('keyword').value)
        + '&type=' + document.getElementById('type').value, {headers:{'X-Requested-With':'XMLHttpRequest'}})
        ...
        .then(function(res){
            ...
            if(!res.data || !res.data.length){
                ...
                res.data.forEach(function(l){
                    var t = typeMap[l.type] || [l.type, 'tag-gray'];
                    var amountHtml = l.amount > 0
                        ? '<span class="price">+' + l.amount + '</span>'
                        : '<span style="color:#dc2626;font-weight:bold;">' + l.amount + '</span>';
                    html += '<tr>'
                        + '<td>' + l.id + '</td>'
                        + '<td><a href="/admin1314/member/detail?id=' + l.user_id + '">' + (l.nickname || l.mobile || l.user_id) + '</a></td>'
                        + '<td><span class="tag ' + t[1] + '">' + t[0] + '</span></td>'
                        + '<td>' + amountHtml + '</td>'
                        + '<td>￥' + l.balance + '</td>'
                        + '<td>' + l.remark + '</td>'
                        + '<td>' + fmtTime(l.create_time) + '</td></tr>';
                });
            }

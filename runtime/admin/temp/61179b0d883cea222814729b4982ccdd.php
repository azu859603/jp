<?php /*a:2:{s:63:"/www/wwwroot/2026/08/16/17_3/app/admin/view/recharge/index.html";i:1787132193;s:55:"/www/wwwroot/2026/08/16/17_3/app/admin/view/layout.html";i:1787106485;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>充值审核 - 竞拍商城</title>
<style>
...
<div class="layout">
    ...
    <div class="main">
        <header class="topbar">
            <div class="breadcrumb">财务管理 / <b>充值审核</b></div>
...
<div class="card">
    <div class="table-wrap">
        <table class="table">
            <thead>
            <tr>
                <th>ID</th><th>会员</th><th>充值金额</th><th>状态</th><th>申请时间</th><th>操作</th>
            </tr>
            </thead>
...
function loadList(page){
    currentPage = page;
    fetch('/admin1314/recharge/index?page=' + page + '&limit=15&status=' + document.getElementById('status').value, {headers:{'X-Requested-With':'XMLHttpRequest'}})
            ...
            if(!res.data || !res.data.length){
                ...
                res.data.forEach(function(w){
                    var stHtml = w.status == 0 ? '<span class="tag tag-orange">待审核</span>'
                        : (w.status == 1 ? '<span class="tag tag-green">已到账</span>' : '<span class="tag tag-red">已拒绝</span>');
                    var ops = '<a class="btn btn-sm" href="/admin1314/member/detail?id=' + w.user_id + '">会员</a> ';
                    if(w.status == 0){
                        ops += '<a class="btn btn-sm btn-green" href="javascript:;" onclick="audit(' + w.id + ',\'pass\')">到账</a> '
...
function audit(id, action){
    confirmDialog('确认后会员余额将到账该笔金额，确定到账吗？', function(){
        ajaxPost('/admin1314/recharge/audit', {id:id, action:action}, function(res){
...
function doReject(){
    ...
    ajaxPost('/admin1314/recharge/audit', {id:currentId, action:'reject', reason:reason}, function(res){
        alert(res.msg);
        if(res.code == 1){

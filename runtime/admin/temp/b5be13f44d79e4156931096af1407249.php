<?php /*a:2:{s:60:"/www/wwwroot/2026/08/16/17_3/app/admin/view/index/index.html";i:1787106492;s:55:"/www/wwwroot/2026/08/16/17_3/app/admin/view/layout.html";i:1787106485;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>系统概览 - 竞拍商城</title>
<style>
...
<div class="layout">
    ...
    <div class="main">
        <header class="topbar">
            <div class="breadcrumb">首页 / <b>系统概览</b></div>
...
<div class="stat-grid">
    <div class="stat-card">
        ...
        <div class="stat-info"><div class="num"><?php echo htmlentities((string) $stats['user_count']); ?></div><div class="label">会员总数</div></div>
    </div>
    <div class="stat-card">
        <div class="stat-icon bg-purple">🏪</div>
        <div class="stat-info"><div class="num"><?php echo htmlentities((string) $stats['seller_count']); ?></div><div class="label">卖家数量</div></div>
    </div>
    <div class="stat-card">
        <div class="stat-icon bg-blue">📦</div>
        <div class="stat-info"><div class="num"><?php echo htmlentities((string) $stats['goods_count']); ?></div><div class="label">商品总数</div></div>
    </div>
    <div class="stat-card">
        <div class="stat-icon bg-green">🔨</div>
        <div class="stat-info"><div class="num"><?php echo htmlentities((string) $stats['auctioning']); ?></div><div class="label">竞拍中</div></div>
    </div>
    <div class="stat-card">
        <div class="stat-icon bg-red">🧾</div>
        <div class="stat-info"><div class="num"><?php echo htmlentities((string) $stats['order_count']); ?></div><div class="label">订单总数</div></div>
    </div>
    <div class="stat-card">
        <div class="stat-icon bg-cyan">💰</div>
        <div class="stat-info"><div class="num">￥<?php echo htmlentities((string) $stats['total_amount']); ?></div><div class="label">成交总额</div></div>
    </div>
    <div class="stat-card">
        <div class="stat-icon bg-orange">📈</div>
        <div class="stat-info"><div class="num">￥<?php echo htmlentities((string) $stats['total_commission']); ?></div><div class="label">平台佣金</div></div>
    </div>
</div>
...
<div class="card">
    ...
    <table class="table">
        <tr>
            <td>商品待审核 <b class="price"><?php echo htmlentities((string) $stats['pending_check']); ?></b> 个</td>
            <td><a class="btn btn-sm" href="/admin1314/goods/check">去处理</a></td>
        </tr>
        <tr>
            <td>卖家开通待审核 <b class="price"><?php echo htmlentities((string) $stats['pending_seller']); ?></b> 个</td>
            <td><a class="btn btn-sm" href="/admin1314/member/seller">去处理</a></td>
        </tr>
        <tr>
            <td>待打款订单 <b class="price"><?php echo htmlentities((string) $stats['unpaid_order']); ?></b> 个</td>
            <td><a class="btn btn-sm" href="/admin1314/order/index?pay_status=0">去处理</a></td>
        </tr>
        <tr>
            <td>提现申请待审核 <b class="price"><?php echo htmlentities((string) $stats['pending_withdraw']); ?></b> 笔</td>
            <td><a class="btn btn-sm" href="/admin1314/withdraw/index?status=0">去处理</a></td>
        </tr>
    </table>
</div>
...
<!-- 近7天成交 -->
<div class="card">
    <div class="card-title">近7天成交走势</div>
    <div style="display:flex;align-items:flex-end;gap:18px;height:180px;padding:10px 6px 0;">
        <?php if(is_array($trend) || $trend instanceof \think\Collection || $trend instanceof \think\Paginator): $i = 0; $__LIST__ = $trend;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$t): $mod = ($i % 2 );++$i;?>
        <div style="flex:1;display:flex;flex-direction:column;align-items:center;height:100%;justify-content:flex-end;">
            <div style="font-size:12px;color:#ff7a00;font-weight:bold;margin-bottom:4px;"><?php echo htmlentities((string) $t['amount']); ?></div>
            <div style="width:100%;max-width:46px;background:linear-gradient(180deg,#ffb066,#ff7a00);border-radius:6px 6px 0 0;height:<?php echo htmlentities((string) $t['height']); ?>px;min-height:4px;" title="成交<?php echo htmlentities((string) $t['count']); ?>单"></div>
            <div style="font-size:12px;color:#94a3b8;margin-top:6px;"><?php echo htmlentities((string) $t['date']); ?></div>
        </div>
        <?php endforeach; endif; else: echo "" ;endif; ?>
    </div>
</div>
...
<div class="card">
    ...
    <div class="table-wrap">
        <table class="table">
            <thead>
            <tr><th>订单号</th><th>商品</th><th>买家</th><th>成交价</th><th>佣金</th><th>状态</th><th>时间</th></tr>
            </thead>
            <tbody>
            <?php if(empty($recentOrders) || (($recentOrders instanceof \think\Collection || $recentOrders instanceof \think\Paginator ) && $recentOrders->isEmpty())): ?>
            <tr><td colspan="7" class="empty">暂无订单数据</td></tr>
            <?php else: if(is_array($recentOrders) || $recentOrders instanceof \think\Collection || $recentOrders instanceof \think\Paginator): $i = 0; $__LIST__ = $recentOrders;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$o): $mod = ($i % 2 );++$i;?>
            <tr>
                <td><?php echo htmlentities((string) $o['order_no']); ?></td>
                <td style="max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"><?php echo htmlentities((string) $o['goods_title']); ?></td>
                <td><?php echo htmlentities((string) $o['buyer_id']); ?></td>
                <td class="price">￥<?php echo htmlentities((string) $o['price']); ?></td>
                <td>￥<?php echo htmlentities((string) $o['commission']); ?></td>
                <td>
                    <?php if($o['order_status'] == 0): ?><span class="tag tag-orange">待付款</span>
                    <?php elseif($o['order_status'] == 1): ?><span class="tag tag-blue">待发货</span>
                    <?php elseif($o['order_status'] == 2): ?><span class="tag tag-purple">待收货</span>
                    <?php elseif($o['order_status'] == 3): ?><span class="tag tag-green">已完成</span>
                    <?php else: ?><span class="tag tag-gray">已取消</span>
                    <?php endif; ?>
                </td>
                <td><?php echo htmlentities((string) date('Y-m-d H:i',!is_numeric($o['create_time'])? strtotime($o['create_time']) : $o['create_time'])); ?></td>
            </tr>
            <?php endforeach; endif; else: echo "" ;endif; ?>
            <?php endif; ?>
            </tbody>
...
<div class="card">
    <div class="card-title">最新出价</div>
    <div class="table-wrap">
        <table class="table">
            <thead>
            <tr><th>商品</th><th>出价人</th><th>出价</th><th>状态</th><th>时间</th></tr>
            </thead>
            <tbody>
            <?php if(empty($recentBids) || (($recentBids instanceof \think\Collection || $recentBids instanceof \think\Paginator ) && $recentBids->isEmpty())): ?>
            <tr><td colspan="5" class="empty">暂无出价记录</td></tr>
            <?php else: if(is_array($recentBids) || $recentBids instanceof \think\Collection || $recentBids instanceof \think\Paginator): $i = 0; $__LIST__ = $recentBids;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$b): $mod = ($i % 2 );++$i;?>
            <tr>
                <td style="max-width:220px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"><?php echo htmlentities((string) $b['goods_title']); ?></td>
                ...
                <td>
                    <?php if($b['is_winner'] == 1): ?><span class="tag tag-green">得标</span>
                    <?php elseif($b['status'] == 2): ?><span class="tag tag-gray">已退回</span>
                    <?php else: ?><span class="tag tag-blue">竞拍中</span>
                    <?php endif; ?>
                </td>
                <td><?php echo htmlentities((string) date('Y-m-d H:i',!is_numeric($b['create_time'])? strtotime($b['create_time']) : $b['create_time'])); ?></td>
            </tr>
            <?php endforeach; endif; else: echo "" ;endif; ?>
            <?php endif; ?>
            </tbody>

<?php /*a:2:{s:50:"D:\work\08\17_3\app\admin\view\category\index.html";i:1787106164;s:42:"D:\work\08\17_3\app\admin\view\layout.html";i:1787106090;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>分类管理 - 竞拍商城</title>
<style>
...
<div class="layout">
    ...
    <div class="main">
        <header class="topbar">
            <div class="breadcrumb">商品管理 / <b>分类管理</b></div>
...
<div class="page-title">
    <span>商品分类</span>
    <div class="tools">
        <button class="btn btn-primary" onclick="openEdit(0)">+ 新增分类</button>
    </div>
</div>
...
<div class="card">
    <div class="table-wrap">
        <table class="table">
            <thead>
            <tr><th>ID</th><th>分类名称</th><th>图标</th><th>排序</th><th>商品数</th><th>状态</th><th>操作</th></tr>
            </thead>
            <tbody>
            <?php if(empty($list) || (($list instanceof \think\Collection || $list instanceof \think\Paginator ) && $list->isEmpty())): ?>
            <tr><td colspan="7" class="empty">暂无分类</td></tr>
            <?php else: if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$c): $mod = ($i % 2 );++$i;?>
            <tr>
                <td><?php echo htmlentities((string) $c['id']); ?></td>
                <td><?php echo htmlentities((string) $c['name']); ?></td>
                <td><?php if(!empty($c['image'])): ?><img src="<?php echo htmlentities((string) $c['image']); ?>" style="width:36px;height:36px;border-radius:4px;object-fit:cover;vertical-align:middle;"><?php else: ?><?php echo !empty($c['icon']) ? htmlentities((string) $c['icon']) : '-'; ?><?php endif; ?></td>
                <td><?php echo htmlentities((string) $c['sort']); ?></td>
                <td><?php echo htmlentities((string) $c['goods_count']); ?></td>
                <td><?php if($c['status'] == 1): ?><span class="tag tag-green">显示</span><?php else: ?><span class="tag tag-gray">隐藏</span><?php endif; ?></td>
                <td style="white-space:nowrap;">
                    <a class="btn btn-sm" href="javascript:;" onclick="openEdit(<?php echo htmlentities((string) $c['id']); ?>)">编辑</a>
                    <a class="btn btn-sm btn-danger" href="javascript:;" onclick="del(<?php echo htmlentities((string) $c['id']); ?>)">删除</a>
                </td>
            </tr>
            <?php endforeach; endif; else: echo "" ;endif; ?>
            <?php endif; ?>
...
<div class="mask" id="editMask">
    <div class="dialog">
        <div class="dialog-title" id="editTitle">新增分类</div>
        <div class="dialog-body">
            ...
            <div class="form-group">
                <label>分类图片（建议正方形，首页展示）</label>
                <div class="upload-box">
                    <div class="up-item add" id="cateImgAdd" onclick="uploadCateImg()">+</div>
                </div>
                <div class="tip">上传后首页分类方块将展示该图片，不传则显示图标/首字</div>
                <input type="hidden" id="editImage" value="">
            </div>
            <div class="form-group">
                <label>图标（可选，无图片时兜底展示，支持Emoji）</label>
                <input type="text" class="form-control" id="editIcon" placeholder="例如：📱">
            </div>
            <div class="form-row">
                <div class="form-group">
...
function setCateImg(url){
    var box = document.getElementById('cateImgBox');
    if(!box){
        box = document.createElement('div');
        box.className = 'up-item';
        box.id = 'cateImgBox';
        document.getElementById('cateImgAdd').parentNode.insertBefore(box, document.getElementById('cateImgAdd'));
    }
    box.innerHTML = '<img src="'+url+'"><span class="del" onclick="removeCateImg()">×</span>';
    box.onclick = function(){ uploadCateImg(); };
    document.getElementById('editImage').value = url;
}

function removeCateImg(){
    var box = document.getElementById('cateImgBox');
    if(box) box.remove();
    document.getElementById('editImage').value = '';
}

function uploadCateImg(){
    ...
    input.onchange = function(e){
        ...
        var fd = new FormData();
        fd.append('file', file);
        fetch('/admin1314/category/upload', {method:'POST', headers:{'X-Requested-With':'XMLHttpRequest'}, body:fd})
...
function doSave(){
    ...
    ajaxPost('/admin1314/category/save', data, function(res){
        alert(res.msg);
...
function del(id){
    confirmDialog('确定删除该分类吗？', function(){
        ajaxPost('/admin1314/category/delete', {id:id}, function(res){

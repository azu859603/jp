<?php /*a:2:{s:62:"/www/wwwroot/2026/08/16/17_3/app/admin/view/member/seller.html";i:1787106497;s:55:"/www/wwwroot/2026/08/16/17_3/app/admin/view/layout.html";i:1787106485;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>卖家审核 - 竞拍商城</title>
<style>
...
<div class="layout">
    ...
    <div class="main">
        <header class="topbar">
            <div class="breadcrumb">会员管理 / <b>卖家审核</b></div>
...
<div class="search-bar">
    <select class="form-control" id="status" onchange="loadList(1)">
        <option value="" selected>全部申请记录</option>
...
<div class="card">
    <div class="table-wrap">
        <table class="table">
            <thead>
            <tr>
                <th>ID</th><th>申请人</th><th>店铺/企业</th><th>营业执照</th><th>注册时间</th><th>当前状态</th><th>操作</th>
            </tr>
            </thead>
...
function loadList(page){
    currentPage = page;
    fetch('/admin1314/member/seller?page=' + page + '&limit=15&status=' + document.getElementById('status').value, {headers:{'X-Requested-With':'XMLHttpRequest'}})
        ...
        .then(function(res){
            ...
            if(!res.data || !res.data.length){
                ...
                res.data.forEach(function(u){
                    var st = u.seller_check;
                    var stHtml = st == 0 ? '<span class="tag tag-orange">待审核</span>'
                        : (st == 1 ? '<span class="tag tag-green">已通过</span>' : '<span class="tag tag-red">已拒绝</span>');
                    var licHtml = '-';
                    if (u.license_img) {
                        var arr = String(u.license_img).split(',').filter(function(s){ return s; });
                        licHtml = arr.map(function(p){
                            return '<img src="' + p + '" style="height:34px;border-radius:4px;cursor:pointer;margin-right:4px;" onclick="window.open(\'' + p + '\')">';
                        }).join('');
                    }
                    html += '<tr>'
                        + '<td>' + u.id + '</td>'
                        + '<td>' + (u.nickname || '-') + '<br><span style="color:#94a3b8;font-size:12px;">' + u.mobile + (u.real_name ? ' ｜ ' + u.real_name : '') + '</span></td>'
                        + '<td>' + (u.shop_name || '-') + '<br><span style="color:#94a3b8;font-size:12px;">' + (u.company_name || '') + '</span></td>'
                        + '<td>' + licHtml + '</td>'
                        + '<td>' + (u.reg_time ? fmtTime(u.reg_time) : '-') + '</td>'
                        + '<td>' + stHtml + '</td>'
                        + '<td style="white-space:nowrap;">'
                        + '<a class="btn btn-sm" href="/admin1314/member/detail?id=' + u.id + '">详情</a> '
                        + (st == 0 ? '<a class="btn btn-sm btn-green" href="javascript:;" onclick="audit(' + u.id + ',\'pass\')">通过</a> '
...
function audit(id, action){
    var tip = action == 'pass' ? '通过后该会员将获得卖家权限，确定通过吗？' : '确定拒绝该会员的卖家申请吗？';
    confirmDialog(tip, function(){
        ajaxPost('/admin1314/member/sellerAudit', {id:id, action:action}, function(res){

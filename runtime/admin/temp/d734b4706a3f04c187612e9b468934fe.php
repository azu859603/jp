<?php /*a:2:{s:47:"D:\work\08\17_3\app\admin\view\goods\check.html";i:1787183436;s:42:"D:\work\08\17_3\app\admin\view\layout.html";i:1787106485;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>商品审核 - 竞拍商城</title>
<style>
...
<div class="layout">
    ...
    <div class="main">
        <header class="topbar">
            <div class="breadcrumb">商品管理 / <b>商品审核</b></div>
...
<div class="search-bar">
    ...
    <span class="tip">通过后商品将进入拍卖中，请仔细审核商品信息</span>
</div>
...
<div class="card">
    <div class="table-wrap">
        <table class="table">
            <thead>
            <tr>
                <th>ID</th><th>封面</th><th>商品标题</th><th>卖家</th><th>起拍价</th>
                <th>加价幅度</th><th>保证金</th><th>拍卖时间</th><th>状态</th><th>操作</th>
            </tr>
            </thead>
...
function loadList(page){
    currentPage = page;
    fetch('/admin1314/goods/check?page=' + page + '&limit=15&status=' + document.getElementById('status').value, {headers:{'X-Requested-With':'XMLHttpRequest'}})
            ...
            if(!res.data || !res.data.length){
                ...
                res.data.forEach(function(g){
                    ...
                    html += '<tr>'
                        ...
                        + '<td>￥' + g.raise_price + '</td>'
                        + '<td>￥' + g.deposit + '</td>'
                        + '<td>' + (g.start_time ? fmtTime(g.start_time) : '-') + '<br>' + (g.end_time ? fmtTime(g.end_time) : '-') + '</td>'
                        + '<td><span class="tag ' + (g.status == 0 ? 'tag-orange">待审核' : 'tag-red">已拒绝') + '</span></td>'
                        + '<td style="white-space:nowrap;">'
                        + '<a class="btn btn-sm" href="/admin1314/goods/detail?id=' + g.id + '">详情</a> '
                        + (g.status == 5
                            ? '<a class="btn btn-sm btn-danger" href="javascript:;" onclick="delGoods(' + g.id + ')">删除</a>'
...
function audit(id, action){
    confirmDialog('通过后商品将进入拍卖中，确定通过吗？', function(){
...
function delGoods(id){
    confirmDialog('确定删除该商品吗？删除后不可恢复', function(){

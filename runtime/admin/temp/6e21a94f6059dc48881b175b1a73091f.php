<?php /*a:2:{s:59:"/www/wwwroot/2026/08/16/17_3/app/admin/view/news/index.html";i:1787106498;s:55:"/www/wwwroot/2026/08/16/17_3/app/admin/view/layout.html";i:1787106485;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>新闻管理 - 竞拍商城</title>
<style>
...
<div class="layout">
    ...
    <div class="main">
        <header class="topbar">
            <div class="breadcrumb">系统设置 / <b>新闻管理</b></div>
...
<div class="page-title">
    <span>新闻公告</span>
    <div class="tools">
        <button class="btn btn-primary" onclick="openEdit(0)">+ 发布新闻</button>
    </div>
</div>

<div class="card">
    <div class="card-head">
        <form class="filter-form" onsubmit="doFilter();return false;">
            <input type="text" class="form-control" style="width:220px;" id="filterKeyword" placeholder="搜索标题" value="<?php echo htmlentities((string) $keyword); ?>">
            <select class="form-control" style="width:130px;" id="filterStatus">
                <option value="-1" <?php if($status==-1): ?>selected<?php endif; ?>>全部状态</option>
    ...
    <div class="table-wrap">
        <table class="table">
            <thead>
            <tr><th>ID</th><th>标题</th><th>内容预览</th><th>状态</th><th>发布时间</th><th>更新时间</th><th>操作</th></tr>
            </thead>
            <tbody>
            <?php if(empty($list) || (($list instanceof \think\Collection || $list instanceof \think\Paginator ) && $list->isEmpty())): ?>
            <tr><td colspan="7" class="empty">暂无新闻，点击右上角发布</td></tr>
            <?php else: if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$n): $mod = ($i % 2 );++$i;?>
            <tr>
                <td><?php echo htmlentities((string) $n['id']); ?></td>
                <td style="max-width:260px;"><?php echo htmlentities((string) $n['title']); ?></td>
                <td class="gray" style="max-width:280px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"><?php echo htmlentities((string) mb_substr(strip_tags($n['content']),0,30)); ?></td>
                <td><?php if($n['status'] == 1): ?><span class="tag tag-green">显示</span><?php else: ?><span class="tag tag-gray">隐藏</span><?php endif; ?></td>
                <td><?php echo htmlentities((string) date('Y-m-d H:i',!is_numeric($n['create_time'])? strtotime($n['create_time']) : $n['create_time'])); ?></td>
                <td><?php if($n['update_time']>0): ?><?php echo htmlentities((string) date('Y-m-d H:i',!is_numeric($n['update_time'])? strtotime($n['update_time']) : $n['update_time'])); else: ?>-<?php endif; ?></td>
                <td style="white-space:nowrap;">
                    <a class="btn btn-sm" href="javascript:;" onclick="openEdit(<?php echo htmlentities((string) $n['id']); ?>)">编辑</a>
                    <a class="btn btn-sm btn-danger" href="javascript:;" onclick="del(<?php echo htmlentities((string) $n['id']); ?>)">删除</a>
                </td>
            </tr>
            <?php endforeach; endif; else: echo "" ;endif; ?>
            <?php endif; ?>
...
<div class="mask" id="editMask">
    <div class="dialog" style="width:560px;">
        <div class="dialog-title" id="editTitle">发布新闻</div>
        <div class="dialog-body">
            <input type="hidden" id="editId" value="0">
            <div class="form-group">
                <label>新闻标题 <span class="req">*</span></label>
                <input type="text" class="form-control" id="editTitleInput" maxlength="100" placeholder="请输入新闻标题（100字以内）">
            </div>
            <div class="form-group">
                <label>新闻内容 <span class="req">*</span></label>
                <textarea class="form-control" id="editContent" rows="8" placeholder="请输入新闻内容" style="resize:vertical;"></textarea>
            </div>
            <div class="form-row">
                <div class="form-group">
                    <label>状态</label>
...
function openEdit(id){
    ...
    document.getElementById('editTitle').textContent = '发布新闻';
    if(id > 0){
        editData.forEach(function(n){
            if(n.id == id){
                ...
                document.getElementById('editTitle').textContent = '编辑新闻';
            }
        });
    }
...
function doSave(){
    ...
    if(!data.title){ alert('请输入新闻标题'); return; }
    if(!data.content){ alert('请输入新闻内容'); return; }
    ajaxPost('/admin1314/news/save', data, function(res){
        alert(res.msg);
...
function del(id){
    confirmDialog('确定删除该新闻吗？', function(){
        ajaxPost('/admin1314/news/delete', {id:id}, function(res){
...
function doFilter(){
    ...
    location.href = '/admin1314/news/index?status=' + st + '&keyword=' + encodeURIComponent(kw);
}
</script>

...
try {
    [$c, $b] = req($asid, 'GET', '/admin1314/bid/index', null, false); ok('主后台页面含操作列与删除按钮脚本', $c == 200 && strpos($b, '<th>操作</th>') !== false && strpos($b, 'function delBid') !== false && strpos($b, 'colspan="10"') !== false && strpos($b, '<th>买家账号</th>') !== false && strpos($b, '<th>卖家账号</th>') !== false, "HTTP $c");
    [$c, $b] = req($gsid, 'GET', '/agent/bid/index', null, false); ok('代理端页面含操作列与删除按钮脚本', $c == 200 && strpos($b, '<th>操作</th>') !== false && strpos($b, 'function delBid') !== false && strpos($b, 'colspan="11"') !== false && strpos($b, '<th>买家账号</th>') !== false && strpos($b, '<th>卖家账号</th>') !== false, "HTTP $c");
    ...
    [$c, $b] = req($bsid, 'GET', '/user/balance_log', null, false); ok('B 的前台余额明细显示退回记录', $c == 200 && strpos($b, '出价记录删除，保证金退回') !== false, "HTTP $c");

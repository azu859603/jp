...
/**
 * 随机中文昵称：四种风格各取两段词库组合，约 780 种，贴近收藏 / 竞拍人群的真实昵称
 */
function random_cn_nickname()
{
    static $pools = [
        // 斋号风：云水轩、墨香斋
        [['云水', '墨香', '清风', '听雨', '半山', '南山', '闲云', '松石', '竹里', '沉香', '紫檀', '青瓷', '白玉', '金石', '翰墨', '砚田', '兰亭', '柳岸', '荷塘', '梅影', '秋山', '春江', '明月', '寒江', '烟雨', '茶烟', '抱朴', '观复', '知白', '拾遗'],
         ['轩', '斋', '阁', '居', '堂', '园', '庐', '舍', '馆', '榭']],
        // 称呼风：王掌柜、李先生
        [['王', '李', '张', '刘', '陈', '杨', '黄', '赵', '周', '吴', '徐', '孙', '马', '朱', '胡', '郭', '林', '何', '高', '罗', '郑', '梁', '谢', '宋', '唐', '许', '邓', '冯', '韩', '曹'],
         ['先生', '老师', '大哥', '掌柜', '藏友', '老板', '师傅', '同学']],
        // 性格风：资深藏家、佛系玩家
        [['资深', '低调', '快乐', '幸运', '佛系', '热心', '专业', '认真', '随缘', '淡定', '老练', '稳重', '神秘', '执着', '安静'],
         ['藏家', '玩家', '买手', '收藏者', '鉴赏家', '爱好者', '淘客', '拍友']],
        // 地域风：江南藏家、沪上淘客
        [['江南', '京城', '岭南', '姑苏', '金陵', '沪上', '蜀中', '齐鲁', '燕赵', '洛城', '杭城', '羊城', '申城', '三晋', '荆楚', '西泠', '徽州', '关中', '塞北', '海上'],
         ['藏家', '玩家', '收藏者', '老藏友', '淘客', '拍客']],
    ];
    $p = $pools[array_rand($pools)];
    return $p[0][array_rand($p[0])] . $p[1][array_rand($p[1])];
}

/**
 * 批量添加虚拟会员用的昵称
 * - 填了前缀：保持「前缀 + 4 位随机数字」，方便按批次识别
 * - 留空：随机中文昵称，如 云水轩 / 王掌柜 / 江南藏家
 * $used 是已占用昵称的集合（键为昵称），生成后写回，避免同批次内重复
 */
function generate_virtual_nickname($prefix = '', array &$used = [])
{
    $prefix = trim((string)$prefix);
    if ($prefix !== '') {
        $name = mb_substr($prefix, 0, 20) . str_pad((string)mt_rand(0, 9999), 4, '0', STR_PAD_LEFT);
        $used[$name] = 1;
        return $name;
    }
    for ($i = 0; $i < 30; $i++) {
    ...
    // 词库被占满时补随机数字，如 云水轩18
    do {
        $name = random_cn_nickname() . mt_rand(2, 999);

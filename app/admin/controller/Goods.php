...
class Goods extends Base
{
    ...
    public function setTime()
    {
        ...
        if ($goods['status'] != 0 && $goods['status'] != 1) {
            return json(['code' => 0, 'msg' => '只有竞拍中或待审核的商品可以修改拍卖时间']);
        }
        if ($startTime === '') {
        ...
        admin_log('修改商品拍卖时间：' . $goods['title'] . ' → ' . date('Y-m-d H:i', $st) . ' ~ ' . date('Y-m-d H:i', $et));
    ...
    public function relistFailed()
    {
        ...
        if (!$seller || (int)$seller['is_seller'] !== 1 || (int)$seller['seller_check'] !== 1) {
     ...
     * - 已成交的商品不能编辑
     * - 拍卖中且已有出价的商品，起拍价 / 保证金 / 保留价锁定不可改（加价幅度允许调整，只影响后续出价）
    ...
    public function edit()
    {
        ...
        admin_log('编辑商品：' . $title . '（ID:' . $id . '）' . ($rawViews !== '' && (int)$rawViews !== (int)$goods['view_count'] ? '，浏览量 ' . (int)$goods['view_count'] . ' → ' . (int)$rawViews : ''));

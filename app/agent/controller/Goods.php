...
class Goods extends Base
{
    ...
    public function index()
    {
        ...
        $sellers    = empty($sellerIds) ? [] : Db::name('user')
            ...
            ->field('id,nickname,account as mobile')
            ->order('id', 'desc')
            ->select()
            ->toArray();
    ...
    public function add()
    {
        ...
        agent_log('代发布产品：' . $title . '（卖家 ' . (user_account($seller) ?: $sellerId) . '）');
    ...
    public function relistFailed()
    {
        ...
        agent_log('批量重新上架流拍产品：卖家 ' . (user_account($seller) ?: $sellerId) . '，' . count($ids) . ' 件');
    ...
    public function edit()
    {
        ...
        if ($goods['status'] == 2) {
            return json(['code' => 0, 'msg' => '已成交的产品不能编辑']);
        }
        $title        = trim($this->request->post('title', ''));
        $categoryId   = (int)$this->request->post('category_id', 0);
        $content      = clean_html(trim($this->request->post('content', '')));
